...
class ClientWindow(QMainWindow):
    ...
    def update_image(self):
        ...
        if frame is not None:
            # Print frame type for debugging
            print(f"Received frame type: {type(frame[0])}")

            # Convert PIL Image to bytes
            img_byte_arr = BytesIO()
            frame[0].save(img_byte_arr, format='JPEG')
            img_byte_arr = img_byte_arr.getvalue()

            # Convert bytes to QByteArray
            byte_array = QByteArray(img_byte_arr)
            
            # Create QImage from the JPEG data
            image = QImage()
            image.loadFromData(byte_array, "JPEG")
            
            # Convert QImage to QPixmap
            pixmap = QPixmap.fromImage(image)
            
            # Set the pixmap to the video player
            self.video_player.setPixmap(pixmap)

            # Optional: resize the pixmap to fit the video player if needed
            # self.video_player.setPixmap(pixmap.scaled(self.video_player.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation))
